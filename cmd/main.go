...
import (
	...
	"github.com/achetronic/tunnel/internal/controller"
	"github.com/achetronic/tunnel/internal/logging"
	"github.com/achetronic/tunnel/internal/version"
...
func main() {
	...
	flag.StringVar(&envoyVersion, "envoy-version", controller.DefaultEnvoyVersion,
		"Envoy release installed on every managed VPS (downloaded from the official Envoy GitHub releases).")
	var tunnelctlDir string
	flag.StringVar(&tunnelctlDir, "tunnelctl-dir", controller.DefaultTunnelctlDir,
		"Directory holding the static tunnelctl binaries (tunnelctl-linux-<arch>) pushed to each VPS over SSH.")
	var imageRepo string
	flag.StringVar(&imageRepo, "image-repo", controller.DefaultImageRepo,
		"Base repository for operator-managed images; the uplink image is <repo>/uplink:<tag>.")
	var imageTag string
	...
	// manager, the reconcilers and the lower-level provision logs share one
	// consistent stream (text or JSON, level from LOG_FORMAT/LOG_LEVEL).
	ctrl.SetLogger(logr.FromSlogHandler(logging.SetupDefault()))
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:        "6f1468a9.achetronic.com",
	...
	if err := (&controller.PortBindingReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "portbinding")
		os.Exit(1)
	}
