...
| `internal/agentrun` | Shared apply/status/run core behind `tunnelctl` (apply, watch + re-apply, readiness), used by both the edge oneshot and the uplink daemon. A failed initial apply retries in the background with backoff (1s..60s) until any apply succeeds. Readiness gates on a successful full apply (WireGuard and nftables) plus a live config watcher and a fresh handshake; if the watcher dies the process exits so the kubelet restarts the pod. |
