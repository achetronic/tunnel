...
2. **SSH Enrollment**: Idempotent. Pushes the matching static `tunnelctl` binary to the VPS (read from the local `--tunnelctl-dir`: baked into the operator image, or a local build dir under `make run`; the right arch chosen from `uname -m`), plus the static Envoy binary; writes the relay `tunnelctl` desired-state document and applies it with `tunnelctl apply` (WireGuard via netlink, no `wireguard-tools`, no `wg-quick`, no download on the VPS). The only systemd units installed are the Envoy service and the `tunnel-boot.service` boot oneshot (`Type=oneshot`, `RemainAfterExit=yes`) that reapplies the relay desired-state document on boot (WireGuard plus any `netdev` tuning) so the relay survives a VPS reboot; the Envoy unit `Requires`/`After` it. Neither is a resident daemon. The unit is named `tunnel-boot` because it reapplies the whole node document, not just WireGuard; the WireGuard *interface* it brings up is named `wg-relay`. It also writes the immutable Envoy bootstrap plus LDS/CDS files, any TLS material for edge-terminating bindings, and `state.json` with artifact hashes (relay document, tunnelctl binary hash, LDS, CDS, TLS).
...
#### 4.1.2 Host-level performance tuning
The optional `spec.host` block tunes the VPS as a machine (kernel and NIC), independent of the proxy. The planner defaults and threads it; nothing here is per-listener and there is no generic escape-hatch.
- **`kernelMaxSocketBufferBytes` (socket-buffer ceiling, default 25MB).** One knob wired to two layers, so the user sets a single number. (1) The operator writes the kernel ceiling `net.core.rmem_max`/`wmem_max` into the sysctl drop-in `/etc/sysctl.d/99-tunnel.conf` over SSH during enrollment (alongside `ip_forward`), persisted natively across reboots like any sysctl drop-in. (2) The Envoy render sets `SO_RCVBUF`/`SO_SNDBUF` (SOL_SOCKET, `STATE_PREBIND`) on the UDP listeners only, up to that ceiling. TCP autotunes below the ceiling on its own, so it gets no explicit socket options; UDP does not autotune, so the explicit reclaim is what makes the ceiling effective for it.
- **`disableNicOffloads` (GRO/GSO toggle, default false).** Turns GRO and GSO off on the underlay NIC carrying the WireGuard transport, auto-detected via a netlink route lookup to a public IP (never the `wg-relay` tunnel device, never a hardcoded name). It is applied natively over ethtool-netlink by `tunnelctl` as the `netdev` section of the relay desired-state document (no `ethtool` CLI on the host), so the `tunnel-boot` oneshot reapplies it on reboot. Default off because disabling offloads degrades throughput for ordinary TCP/QUIC traffic by raising per-packet CPU; it is for encapsulated, non-uniform UDP (tunnel-in-tunnel) where GRO coalescing corrupts datagram boundaries and breaks Envoy's L4 datagram semantics.
- **Envoy render (no knob).** GRO is delegated to the OS (controlled at the NIC via `disableNicOffloads`), so the UDP listeners carry no `prefer_gro`. Every listener sets `enable_reuse_port: true` explicitly (honored for both TCP and UDP on Linux; the `reuse_port` field is deprecated).
- **Teardown does not restore live values.** Like `ip_forward`, teardown removes the persistence (the sysctl drop-in file and the `tunnel-boot` unit that reapplies the `netdev` section) and lets the live kernel/NIC values revert on the next reboot, rather than capturing and restoring prior state.
